...
@ZenRegister
public class ConstantCubeRenderers implements IBracketHandler {
    private final static Map<String, IICubeRenderer> cache = new HashMap<>();
    private final IJavaMethod method;

    public ConstantCubeRenderers() {
        this.method = CraftTweakerAPI.getJavaMethod(ConstantCubeRenderers.class, "get", String.class);
    ...
    public IZenSymbol resolve(IEnvironmentGlobal environment, List<Token> tokens) {
        ...
        if (!tokens.get(0).getValue().equalsIgnoreCase("cubeRenderer")) return null;
