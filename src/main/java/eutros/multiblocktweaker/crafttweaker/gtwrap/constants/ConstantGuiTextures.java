...
@ZenRegister
public class ConstantGuiTextures implements IBracketHandler {
    private final static Map<String, ITextureArea> cache = new HashMap<>();

    private final IJavaMethod method;

    public ConstantGuiTextures() {
        this.method = CraftTweakerAPI.getJavaMethod(ConstantGuiTextures.class, "get", String.class);
    ...
    public IZenSymbol resolve(IEnvironmentGlobal environment, List<Token> tokens) {
        ...
        if (!tokens.get(0).getValue().equalsIgnoreCase("gtTexture")) return null;
